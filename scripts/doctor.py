...
def flag(v): return "OK" if v else "MISSING"

def main():
    s = get_settings(); init_db()
    ...
    with session_scope() as db: print("CAFE24_OAUTH_TOKEN", flag(get_token(db, "cafe24")))
    print("SELLMATE", "READY" if s.sellmate_api_base_url and s.sellmate_inventory_endpoint and s.sellmate_api_key else "WAITING FOR API SPEC")
    print("IAPPS", "READY" if s.iapps_api_base_url and s.iapps_daily_endpoint and s.iapps_api_key else "WAITING FOR API SPEC")
