...
def sync_status_bar() -> None:
    ...
    label = {
        ...
        "sellmate": "셀메이트 API",
        "iapps": "아이앱스 API",
        "sellmate_excel": "셀메이트 재고 업로드",
        "iapps_excel": "아이앱스 통계 업로드",
...
def daily_report_guide() -> None:
    with st.expander("MISHARP DAILY REPORT 이용방법 · 처음 세팅할 때 꼭 읽어주세요", expanded=False):
        st.markdown(
            ...
            <div class="mso-guide-step"><b>3. 주요 재고 현황</b><br>실제 재고는 Cafe24 재고를 사용하지 않습니다. 필요할 때 Sellmate에서 내려받은 Excel을 <b>데이터·설정</b>에서 업로드하면 해당 기준일 재고 스냅샷을 교체합니다.</div>
            <div class="mso-guide-step"><b>4. 데이터 기준</b><br>공식 매출·상품 성과는 Cafe24 Analytics, 광고비는 Google Sheet 자동수집을 기준으로 합니다. 실제 재고는 Sellmate Excel 수동업로드, 앱 설치·순방문은 iApps Excel 수동업로드를 기준으로 합니다. SERA는 참고·검증용입니다.</div>
